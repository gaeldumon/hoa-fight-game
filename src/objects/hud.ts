...
export class Hud {
    constructor(scene: Phaser.Scene, user: User) {
        if (user.screenSide === 'left') {
            scene.add.image(0, 0, user.characterInstance.thumbnailKey).setOrigin(0, 0).setScale(0.4);
            ...
            scene.add.image(gameWidth(scene), 0, user.characterInstance.thumbnailKey).setOrigin(1, 0).setScale(0.4);
